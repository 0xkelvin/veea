			       ...
			       const struct i2s_config *i2s_cfg)
{
	...
	if (dir == I2S_DIR_RX || dir == I2S_DIR_BOTH) {
		...
		/* PDM RX mode for I2S0: replace STD with PDM slot/clock/mode.
		 * ESP32-S3 only supports PDM-to-PCM converter on I2S0.
		 * Enabled via CONFIG_I2S_ESP32_PDM_RX Kconfig option.
		 */
#if defined(CONFIG_I2S_ESP32_PDM_RX) && SOC_I2S_SUPPORTS_PDM_RX
		if (dev_cfg->unit == 0) {
			i2s_hal_slot_config_t pdm_slot = {
				.data_bit_width = 16,
				.slot_bit_width = 16,
				.slot_mode = I2S_SLOT_MODE_MONO,
				.pdm_rx = { .slot_mask = I2S_PDM_SLOT_LEFT },
			};
			i2s_hal_pdm_set_rx_slot(hal, false, &pdm_slot);

			i2s_hal_clock_info_t pdm_clk = {
				.sclk = 160000000,
				.mclk = i2s_cfg->frame_clk_freq * 64 * 8,
				.bclk = i2s_cfg->frame_clk_freq * 64,
				.bclk_div = 8,
				.mclk_div = 160000000 / (i2s_cfg->frame_clk_freq * 64 * 8),
			};
			i2s_hal_set_rx_clock(hal, &pdm_clk, I2S_ESP32_CLK_SRC);
